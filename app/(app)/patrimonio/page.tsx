...
export default async function PaginaPatrimonio() {
  ...
  const evolucao = evolucaoPatrimonio(lista, 12);
  const atual = evolucao.at(-1);
  const anterior = evolucao.at(-2);

  return (
    <>
      <h1 className="text-xl font-bold tracking-tight sm:text-2xl">Patrimônio</h1>
      <p className="mt-1 text-sm text-[var(--color-suave)]">
        Registre o saldo de cada conta, investimento e dívida. Cada registro é uma
        foto daquele mês — o gráfico liga os pontos.
      </p>
